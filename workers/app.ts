...
export default {
  async fetch(request, env, ctx) {
    ...
    // FastLink routes (download proxy, viewer, info API, telegram)
    const fastLinkResponse = await handleFastLinkRoutes(request, env, url);
    if (fastLinkResponse) return fastLinkResponse;
